...
class GR1DexEnvInference:
    ...
    def step(self, action_list):
        ...
        if self.use_image:
            obs_dict["image"] = (
                torch.from_numpy(obs_img).permute(0, 3, 1, 2).unsqueeze(0)
            )
    ...
    def reset(self, first_init=True):
        ...
        # pos init
        qpos_init1 = np.array(
            [-np.pi / 12, 0, 0, -1.6, 0, 0, 0, -np.pi / 12, 0, 0, -1.6, 0, 0, 0]
        )
        qpos_init2 = np.array(
            [-np.pi / 12, 0, 1.5, -1.6, 0, 0, 0, -np.pi / 12, 0, -1.5, -1.6, 0, 0, 0]
        )
...
def main(cfg: OmegaConf):
    ...
    if record_data:
        ...
        if choice == "y":
            ...
            os.rename(
                src=record_file_name,
                dst=record_file_name.replace("demo.h5", renamed + ".h5"),
            )
