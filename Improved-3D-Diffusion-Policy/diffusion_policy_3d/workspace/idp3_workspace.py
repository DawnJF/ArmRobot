...
from diffusion_policy_3d.policy.diffusion_pointcloud_policy import (
    DiffusionPointcloudPolicy,
)
...
class iDP3Workspace(BaseWorkspace):
    ...
    def run(self):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in tqdm.tqdm(
                range(cfg.training.num_epochs), desc=f"Training"
            ):
                step_log = dict()
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        step_log["test_mean_score"] = -step_log[
